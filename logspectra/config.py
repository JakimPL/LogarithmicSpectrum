...
class FFTConfig(BaseModel):
    ...
    cutoff: float = Field(default=220.0, gt=0.0, description="Cutoff frequency in Hz for lower bands")
    log_even_components: int = Field(default=80, gt=0, description="Number of log-even frequency components")
    bins_per_octave: int = Field(default=BINS_PER_OCTAVE, gt=0, description="Number of bins per octave for CQT")


class Sampling(BaseModel):
    duration: float = Field(default=0.1, gt=0.0, description="Duration of the signal in seconds")
    rate: int = Field(default=44100, gt=0, description="Sampling rate in Hz")
...
class WaveDefinition(BaseModel):
    ...
    base_frequency: float = Field(default=440.0, gt=0.0, description="Base frequency of the wave in Hz")
